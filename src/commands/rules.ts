...
function printRulesResult(result: LinkResult, source: string, target: string): void {
  if (result.status === 'already_linked') {
    console.log(`${pc.blue('ℹ')} Rules directory ${pc.bold(target)} already points to ${pc.cyan(result.linkValue)}`);
  } else if (result.status === 'created') {
    console.log(`${pc.green('✓')} Linked rules: ${pc.bold(target)} -> ${pc.cyan(result.linkValue)} ${pc.dim(`(from ${source})`)}`);
  } else if (result.status === 'replaced') {
    console.log(`${pc.yellow('✓')} Replaced rules link: ${pc.bold(target)} -> ${pc.cyan(result.linkValue)} ${pc.dim(`(from ${source})`)}`);
  } else if (result.status === 'would_create' || result.status === 'would_replace') {
    console.log(`${pc.magenta('[dry-run]')} Would link rules: ${pc.bold(target)} -> ${pc.cyan(result.linkValue)} ${pc.dim(`(from ${source})`)}`);
  }
}
