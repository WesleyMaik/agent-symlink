...
export async function showMainMenu(): Promise<boolean> {
  const choice = await p.select<MenuAction>({
    ...
    options: [
      ...
      { label: 'Rules / configuration directory', value: 'rules', hint: 'e.g. .agent-config/rules' },
      { label: 'Apply an agent preset', value: 'preset', hint: 'Configure single agent' },
      { label: 'Apply multiple agent presets', value: 'presets_multi', hint: 'Configure multiple agents' },
  ...
  switch (choice) {
    ...
    case 'rules':
      ...
      break;
    case 'preset':
      await runSinglePresetFlow();
      break;
    case 'presets_multi':
      await runMultiplePresetsFlow();
      break;
    case 'inspect':
