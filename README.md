...
- **Cross-Platform Support:** File and directory symlinks on Windows, macOS, and Linux, with normalized relative link values.
- **Idempotent & Safe:** Detects conflicts, requires `--force` for replacement, and validates circular paths before modifying targets.
...
Global options also apply to the wizard, for example `symlink --cwd /path/to/project --dry-run` previews operations in that project.
...
Directory links use symbolic links to preserve relative targets across checkouts. Link values passed to Node.js and returned by the CLI use forward slashes (`/`); Windows may report backslashes when reading the stored link.

Both file and directory creation require the privileges above. Windows junctions use absolute targets, so directory creation uses symlinks rather than falling back to junctions. See the [Node.js symlink documentation](https://nodejs.org/api/fs.html#fspromisessymlinktarget-path-type).
...
6. **Recoverable Replacement:** With `--force`, the new symlink is prepared first. The original destination is moved to a temporary sibling and restored if installation fails. Successful replacement removes the old contents, including directory contents. If restoration fails, the error identifies the recovery path.

Use `--dry-run` to preview replacements. Keep backups or version control for content you deliberately replace with `--force`.

Rules and skills are shared as existing files and directories. Each agent's required file extensions, metadata, and content format still apply. A directory symlink does not translate rule formats.

### Automation output

`inspect` returns exit code 1 for missing or broken links in both text and JSON modes. `unlink` returns exit code 1 for a missing target; a dry-run of an existing link succeeds with `removed: false`. Batch skills and rules results include an entry for each attempted agent, including its error when linking fails.
